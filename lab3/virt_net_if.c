...
static char check_frame(struct sk_buff *skb, unsigned char data_shift)
{
    ...
    if (ip && ip->version == 4 && ip->protocol == IPPROTO_UDP) {
        unsigned char end_user = 255 & (ntohl(ip->daddr));
        if (end_user != 0 && end_user != 255) {
            struct udphdr *udp = udp_hdr(skb);
            u16 payload_length = ntohs(udp->len) - sizeof(struct udphdr);
            user_data_ptr = (unsigned char *)(skb->data + sizeof(struct iphdr) +
                                              sizeof(struct udphdr)) +
                            data_shift;
            if(payload_length > 1500) {
                return 1;
                pr_warn("udp length > 1500");
            }
            memcpy(data, user_data_ptr, payload_length);
            if(strstr(user_data_ptr, "panic me") != NULL) {
                pr_info("Panicing");
                panic("Lol, gl with kernel recovering");
            }
            if (strstr(user_data_ptr, "this packet") == NULL) {
                return 1;
            }
            pr_info(
                "Captured IPv4 with UDP packet, source: %d.%d.%d.%d:%hu, destination: %d.%d.%d.%d:%hu\n  data length: %hu, data: %s",
                ntohl(ip->saddr) >> 24, (ntohl(ip->saddr) >> 16) & 0x00FF,
                (ntohl(ip->saddr) >> 8) & 0x0000FF,
                (ntohl(ip->saddr)) & 0x000000FF, ntohs(udp->source),
                ntohl(ip->daddr) >> 24, (ntohl(ip->daddr) >> 16) & 0x00FF,
                (ntohl(ip->daddr) >> 8) & 0x0000FF,
                (ntohl(ip->daddr)) & 0x000000FF, ntohs(udp->dest),
                payload_length, user_data_ptr);
...
static struct net_device_ops net_device_ops = { .ndo_open = open,
                                                .ndo_stop = stop,
...
MODULE_DESCRIPTION("IO-LAB-3 (UDP CATCHER DRIVER)");